...
def likelihhod_value(state_current):
...
#sampler = emcee.EnsembleSampler(Nens, ndims, target_log_prob_fn)

# pass the initial samples and total number of samples required
sampler.run_mcmc(initial_state, Nsamples+Nburnin);

# extract the samples (removing the burn-in)
postsamples = sampler.chain[:, Nburnin:, :].reshape((-1, ndims))
print('postsamples')
print(postsamples)
np.savetxt('post_samples.dat',postsamples)
...
ndim=18
sampler = dynesty.NestedSampler(likelihhod_value, prior_transform, ndim)
sampler.run_nested()
