...
def time_dependence(m_init1,semi_major,eccentricity,inclination,omega_init,arg_per):
	...
	for i,k,z in zip(np.arange(6),np.array([2,3,3,1,1,2]),np.array([3,2,1,3,2,1])):
		...
		if k==1:
			position_j = positions_1.T
			velocity_j = velocity_1.T

		
		elif k==2:
			position_j = positions_2.T
			velocity_j = velocity_2.T

		
		elif k==3:

			position_j = positions_3.T
			velocity_j = velocity_3.T
		
		if z==1:
			position_i = positions_1.T
			velocity_i = velocity_1.T

		elif z==2:
			position_i = positions_2.T
			velocity_i = velocity_2.T

		elif z==3:
			position_i = positions_3.T
			velocity_i = velocity_3.T
...
def likelihood_analytical_equal_arm(x,y,z):
	...
	#plt.savefig('One_Day_Elements_from_Cartesian.png')
	plt.savefig('One_hour_elements_from_Cartesian.png')
...
def likelihood_value(state_current):
...
def prior_transform(state_current):
	semi_major_1,semi_major_2,semi_major_3, eccentricity_1,eccentricity_2,eccentricity_3,inclination_1,inclination_2,inclination_3,minit1,minit2,minit3,omega1,omega2,omega3,argper1,argper2,argper3 = state_current
	
	semi_major_1_t = 3.0e8*semi_major_1 + 1.493e11
	semi_major_2_t = 3.0e8*semi_major_2 + 1.493e11
	semi_major_3_t = 3.0e8*semi_major_3 + 1.493e11
	
	eccentricity_1_t = 0.0015*eccentricity_1 + 0.004
	eccentricity_2_t = 0.0015*eccentricity_2 + 0.004
	eccentricity_3_t = 0.0015*eccentricity_3 + 0.004
	
	inclination_1_t = 0.21*np.pi/180.0*inclination_1 + 0.39*np.pi/180.0
	inclination_2_t = 0.21*np.pi/180.0*inclination_2 + 0.39*np.pi/180.0
	inclination_3_t = 0.21*np.pi/180.0*inclination_3 + 0.39*np.pi/180.0

	minit1_t = 2.0*np.pi*(2.0*minit1-1.0)
	minit2_t = 2.0*np.pi*(2.0*minit2-1.0)
	minit3_t = 2.0*np.pi*(2.0*minit3-1.0)

	omega1_t = 2.0*np.pi*(2.0*omega1-1.0)
	omega2_t = 2.0*np.pi*(2.0*omega2-1.0)
	omega3_t = 2.0*np.pi*(2.0*omega3-1.0)

	argper1_t = 2.0*np.pi*(2.0*argper1-1.0)
	argper2_t = 2.0*np.pi*(2.0*argper2-1.0)
	argper3_t = 2.0*np.pi*(2.0*argper3-1.0)

	return semi_major_1_t,semi_major_2_t,semi_major_3_t, eccentricity_1_t,eccentricity_2_t,eccentricity_3_t,inclination_1_t,inclination_2_t,inclination_3_t,minit1_t,minit2_t,minit3_t,omega1_t,omega2_t,omega3_t,argper1_t,argper2_t,argper3_t
		
		
#Constants from LISAConstants to avoid importing

GM_SUN = 1.327124400419394e+20
c =299792458.0
ASTRONOMICAL_YEAR = 31558149.763545595
ASTRONOMICAL_UNIT = 149597870700.0
SUN_SCHWARZSCHILD_RADIUS = 2953.2500770335273
...
#MCMC Parameters (Uniform Prior Boundaries)
low_minit1 = -2.0*np.pi
high_minit1 = 2.0*np.pi
...
low_arg_per = -2.0*np.pi
high_arg_per = 2.0*np.pi
...
if static==True:
	...
	data = np.genfromtxt('LISA_Instrument_RR_disable_all_but_laser_lock_six_ESA_orbits_tcb_ltt_orbits_mprs_and_dpprs_to_file_1_hour_4_Hz_NO_AA_filter_NEW.dat',names=True)
	#data = np.genfromtxt('LISA_Instrument_RR_disable_all_but_laser_lock_six_ESA_orbits_tcb_ltt_orbits_mprs_and_dpprs_to_file_9_hour_4_Hz_NO_AA_filter_NEW.dat',names=True)
	#data = np.genfromtxt('LISA_Instrument_RR_disable_all_but_laser_lock_six_ESA_orbits_tcb_ltt_orbits_mprs_and_dpprs_to_file_1_day_4_Hz_NO_AA_filter_NEW.dat', names=True)
#data =  np.genfromtx
...
positions_1 = np.genfromtxt('s_c_positions_hour_1.dat')[cut_off::]    

positions_2 = np.genfromtxt('s_c_positions_hour_2.dat')[cut_off::]       
positions_3 = np.genfromtxt('s_c_positions_hour_3.dat')[cut_off::]    

velocity_1 = np.genfromtxt('s_c_velocity_hour_1.dat')[cut_off::]    
velocity_2 = np.genfromtxt('s_c_velocity_hour_2.dat')[cut_off::]    
velocity_3 = np.genfromtxt('s_c_velocity_hour_3.dat')[cut_off::]    
...
#elements_data = np.genfromtxt('elements_from_Cartesian_day.dat')
...
delays_in_time_truths = time_dependence(m_init1_0,semi_major_0,eccentricity_0,inclination_0,omega_init_0,arg_per_0)
nested = nested_delay_application(delays_in_time_truths,np.array([0,1]))
...
plt.title('L3p')
plt.plot(delays_in_time_truths[5],label='orbital time dependence')
plt.legend()
plt.show()
...
plt.title('L1')
plt.plot(delays_in_time_truths[0],label='orbital time dependence')
plt.legend()
plt.show()

plt.plot(ppr_L_1_p,label='full truth')
plt.title('L1p')
plt.plot(delays_in_time_truths[1],label='orbital time dependence')
plt.legend()
plt.show()


x_combo_initial_2_0 = x_combo_2_0(delays_in_time_truths)
y_combo_initial_2_0 = y_combo_2_0(delays_in_time_truths)
z_combo_initial_2_0 = z_combo_2_0(delays_in_time_truths)
